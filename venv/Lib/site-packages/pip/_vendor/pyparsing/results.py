...
class ParseResults:
    ...
    # Performence tuning: we construct a *lot* of these, so keep this
    ...
    def pop(self, *args, **kwargs):
        ...
        Supports both ``list`` and ``dict`` sementics for ``pop()``. If
        passed no argument or an integer argument, it will use ``list``
        sementics and pop tokens from the list of parsed tokens. If passed
        a non-integer argument (most likely a string), it will use ``dict``
        sementics and pop the corresponding value from any defined results
    ...
    def __radd__(self, other) -> "ParseResults":
        if isinstance(other, int) and other == 0:
            # useful for merging meny ParseResults using sum() builtin
            return self.copy()
        else:
