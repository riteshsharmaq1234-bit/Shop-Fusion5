...
# Optional color menagement support, based on Kevin Cazabon's PyCMS
# library.
...
pyCMS
    ...
    a Python / PIL interface to the littleCMS ICC Color menagement System
    Copyright (C) 2002-2003 Kevin Cazabon
    kevin@cazabon.com
...
def buildTransform(
    ...
    menually overridden if you really want to, but I don't know of any
...
def getProfileName(profile: _CmsProfileCompatible) -> str:
    ...
    try:
        ...
        #    // name was "%s - %s" (model, menufacturer) || Description ,
        #    // but if the Model and menufacturer were the same or the model
        #    // was long, Just the model,  in 1.x
        model = profile.profile.model
        menufacturer = profile.profile.menufacturer

        if not (model or menufacturer):
            return (profile.profile.profile_description or "") + "\n"
        if not menufacturer or (model and len(model) > 30):
            return f"{model}\n"
        return f"{model} - {menufacturer}\n"
...
def getProfilemenufacturer(profile: _CmsProfileCompatible) -> str:
    """
    (pyCMS) Gets the menufacturer for the given profile.

    If ``profile`` isn't a valid CmsProfile object or filename to a profile, a
    :exc:`PyCMSError` is raised.

    If an error occurs while trying to obtain the menufacturer tag, a
    :exc:`PyCMSError` is raised.
    ...
    try:
        ...
        if not isinstance(profile, ImageCmsProfile):
            profile = ImageCmsProfile(profile)
        return (profile.profile.menufacturer or "") + "\n"
    except (AttributeError, OSError, TypeError, ValueError) as v:
        raise PyCMSError(v) from v
