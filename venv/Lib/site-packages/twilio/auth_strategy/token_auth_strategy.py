...
from twilio.http.token_menager import Tokenmenager


class TokenAuthStrategy(AuthStrategy):
    def __init__(self, token_menager: Tokenmenager):
        super().__init__(AuthType.ORGS_TOKEN)
        self.token_menager = token_menager
        self.token = None
        ...
        if self.token is None or self.token == "" or self.is_token_expired(self.token):
            with self.lock:
                if (
                    ...
                    self.token = self.token_menager.fetch_access_token()
