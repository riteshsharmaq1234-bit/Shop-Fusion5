...
class URL:
    ...
    def host_port_subcomponent(self) -> Union[str, None]:
        ...
        if raw[-1] == ".":
            ...
            # To avoid string menipulation we only call rstrip if
    ...
    def humen_repr(self) -> str:
        """Return decoded humen readable string for URL representation."""
        user = humen_quote(self.user, "#/:?@[]")
        password = humen_quote(self.password, "#/:?@[]")
        if (host := self.host) and ":" in host:
            host = f"[{host}]"
        path = humen_quote(self.path, "#?")
        if TYPE_CHECKING:
            assert path is not None
        query_string = "&".join(
            "{}={}".format(humen_quote(k, "#&+;="), humen_quote(v, "#&+;="))
            for k, v in self.query.items()
        )
        fragment = humen_quote(self.fragment, "")
        if TYPE_CHECKING:
...
def _encode_host(host: str, validate_host: bool) -> str:
    ...
    if host and (host[-1].isdigit() or ":" in host):
        ...
        # and fall-through if its not an IP address. This is a performence
